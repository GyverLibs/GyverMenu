...
## Использование
### Минимальный пример
Для псевдо-дисплея LCD1602 `lcd`
...
В качестве точек входа библиотека имеет 5 виртуальных кнопок, которые можно связать с любыми физическими кнопками, джойстиком или энкодером. В качестве точек выхода используются обработчики - библиотека не привязана к конкретному дисплею и способу ввода. Текстовое меню можно вывести на LCD, OLED, терминал или другой интерфейс, а для графического дисплея можно написать свои виджеты.
...
#### Обработчики
- `onBuild(gm::Builder& b)` - построение меню, вызывается библиотекой для разных целей
- `onPrint(const char* str, size_t len)` - вывод текста. `str` может быть `nullptr` - окончание рендера меню. Библиотека всегда ограничивает длину `len` по оставшемуся месту в строке меню. Библиотека всегда печатает строку до конца (до заданной ширины меню), чтобы затереть символы от предыдущих строк при прокрутке
- `onCursor(uint8_t col, uint8_t row)` - установка позиции вывода для следующих вызовов `onPrint` (нужно только для режима частичного обновления)
- `onState(uint8_t row, bool selected, bool editing)` - оформление состояния строки, вызывается в начале строки меню и при смене курсора

Основные сценарии и задачи обработчиков:

1. Только `onState` - простой режим для символьного дисплея или Serial-меню. `onState` сам ставит позицию в начало строки и выводит маркер/состояние. Подходит, когда устройство само двигает курсор после печати. Подходит для режимов обновления `Full` и `Row`

2. `onState` + `onCursor` - основной универсальный режим, позволяет работать с режимом частичного обновления `Part`. `onCursor` полностью отвечает за позиционирование, а `onState` только рисует маркер или переключает состояние строки (например инверсия для OLED)
...
- `Full` - после действия перерисовывается весь экран. Подходит для Serial-терминала, framebuffer или рендера, где частичное обновление не имеет смысла
